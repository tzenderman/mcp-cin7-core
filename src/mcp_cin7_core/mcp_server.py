...
async def cin7_stock_snapshot_start(
    ...
    logger.debug("Tool result: cin7_stock_snapshot_start -> %s", _truncate(str(result)))
    return result
...
async def cin7_stock_snapshot_status(snapshot_id: str) -> Dict[str, Any]:
    ...
    if not snap:
        ...
        logger.debug("Tool result: cin7_stock_snapshot_status -> %s", _truncate(str(result)))
        return result
    result = {
        "snapshotId": snap.id,
        "ready": snap.ready,
...
async def cin7_stock_snapshot_chunk(
    ...
    if not snap:
        result = {"error": "snapshot not found"}
        logger.debug("Tool result: cin7_stock_snapshot_chunk -> %s", _truncate(str(result)))
        return result
    start = max(0, int(offset))
    end = max(start, start + int(limit))
    items = snap.items[start:end]
...
async def cin7_stock_snapshot_close(snapshot_id: str) -> Dict[str, Any]:
    ...
    logger.debug("Tool result: cin7_stock_snapshot_close -> %s", _truncate(str(result)))
    return result
